...
function displayMedia(MediaContainer,MediaBase,MediaURL){
 	...
 	var text='<img align="center" style="display:block;margin-left:auto;margin-right:auto;" usemap="#PnQ" width="'+MediaWidth+'" height="'+MediaHight+'" src="'+MediaBase+MediaURL+'"/>';
	if (MediaURL.toUpperCase().includes("HTTP")==true) {
		text='<img align="center" width="400" style="display:block;margin-left:auto;margin-right:auto;" usemap="#PnQ" src="'+MediaURL+'"/>';
	}
...
//		alert(cmd.xapinote);
	...
	console.log("CURRENT CMD: " + JSON.stringify(cmd));
	...
	//	alert(msg);
	   displayDebugging("#DebuggingArea",msg);
		...
		$("#ASATMainQuestion").css("display","block");
		$("#MQTextHolder").css("display","inline-block");
		$("#MQTextHolder").html(msg.fontsize(afontsize));
		window.msg = msg;
		$("#InputArea").fadeIn();
		$("#inputText").val("");
		$("#inputText").val("");
